...
def recommend_pacbayes_dropout_rate_advanced(
    ...
    c: float = 2.0,  # Default scaling factor for log term (common values: 2, sqrt(n))
    penalty_scale: float = 1.0,  # Allows adjustment (some variants use sqrt(2) or other scale)
    min_penalty: float = 1e-8,   # Prevent sqrt(negative)
) -> Tuple[float, str, str]:
    """
    PAC-Bayes optimal dropout: minimizes the generalization bound with user-supplied
    KL-divergence and empirical loss functions of dropout rate p.

    Args:
        train_loss: Empirical training loss (float)
        N: Model parameter count (int)
        n: Training set size (int)
        delta: Risk/confidence parameter (float in (0,1))
        kl_func: Function KL(q||p) as a function of dropout p (default: (1-p)*N)
        empirical_loss_func: Training loss as a function of dropout p (default: constant)
        p_bounds: Tuple, allowed range for dropout p
        c: Scaling factor in PAC-Bayes bound, see theory (default 2.0)
        penalty_scale: Multiplier on the penalty (default 1.0, some use sqrt(2))
        min_penalty: Minimal penalty added for sqrt safety.

    Returns:
        (best_p, formula_str, explanation_str)

    Theoretical note: Some literature uses log(c/delta) with c=2 or sqrt(n),
    and/or adds "+1" to the numerator for strict bounds.
    """
    ...
    explanation = (
        "Minimizes the PAC-Bayes generalization bound. 'c' and 'penalty_scale' are tunable based on the theoretical variant. "
        "Some bounds use log(2/delta), log(sqrt(n)/delta), or add '+1' for strictness."
    )
...
def recommend_curvature_aware_weight_decay(
    ...
    explanation = (
        "Regularization based on landscape sharpness. "
        "α is a tunable scaling factor. Use a lower bound for safety."
    )
